# -*- coding: utf-8 -*-
"""
Created on Mon Dec 14 10:51:27 2020
...
class Sketcher:
    def __init__(self, windowname, dests,width,hight, colors_func):
        self.prev_pt = None
    ...
    def show(self):
        ...
        cv.resizeWindow(self.windowname,(int(self.width/2), int(self.hight/2)))
        cv.imshow(self.windowname, self.dests[0])
        cv.namedWindow(self.windowname+ ": mask", cv.WINDOW_NORMAL)
        cv.resizeWindow(self.windowname+ ": mask",(int(self.width/2), int(self.hight/2)))
...
def main():
    ...
    path = glob.glob(os.path.join(r'.\static\images\box', '*.jpg'))
    for file in path:
        img = cv.imread(file)
    width = img.shape[0]
    hight = img.shape[1]
    ...
    sketch = Sketcher('image', [img_mask, inpaintMask],width,hight, lambda : ((255, 255, 255), 255))
    ...
    while True:
        ...
        if ch == ord('t'):
            ...
            cv.resizeWindow('Inpaint Output using FMM',width, hight)
        ...
        if ch == ord('n'):
            ...
            cv.resizeWindow('Inpaint Output using NS Technique',width, hight)
