...
import lib.config.config as cfg   #参数
...
def combined_roidb(imdb_names):
    ...
    roidb = roidbs[0]  #roidb=DIY_dataset
    if len(roidbs) > 1:
        for r in roidbs[1:]:
            roidb.extend(r)
...
class Train:
    def __init__(self):
        ...
        if cfg.FLAGS.net == 'vgg16':
            self.net = vgg16(batch_size=cfg.FLAGS.ims_per_batch)  #每个小批量使用的图片数量为1
    ...
    def train(self):
        ...
        with sess.graph.as_default():
            ...
            print('train',self.imdb.num_classes)
            layers = self.net.create_architecture(sess, "TRAIN", self.imdb.num_classes, tag='default') #tag='default'?
...
#                                                                 cfg.FLAGS.pretrained_model)
    ...
    def snapshot(self, sess, iter):
        ...
        filename = 'resnet_v1_50_faster_rcnn_iter_{:d}_3_class'.format(iter) + '.ckpt'
        filename = os.path.join(self.output_dir, filename)
        self.saver.save(sess, filename)
        print('Wrote snapshot to: {:s}'.format(filename))

        # Also store some meta information, random state, etc.
        nfilename = 'resnet_v1_50_faster_rcnn_iter_{:d}_3_class'.format(iter) + '.pkl'
